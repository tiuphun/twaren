...
class TWARENDiscovery:
    ...
    def run_traceroute(self, target: str, use_icmp=True, max_hops=30) -> List[Dict]:
        """Run traceroute and parse output, including partial results before timeout"""
        cmd = ['traceroute']
        if use_icmp:
            cmd.append('-I')
        cmd.extend(['-m', str(max_hops), '-w', '2', target])
        
        try:
            # Use a longer timeout but still capture partial output
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=90)
            return self.parse_traceroute(result.stdout, target)
        except subprocess.TimeoutExpired as e:
            # Still parse whatever we got before timeout
            print(f"  ⚠ Traceroute to {target} timed out, parsing partial results...")
            partial_output = e.stdout.decode() if e.stdout else ""
            return self.parse_traceroute(partial_output, target)
        except Exception as e:
            print(f"  ✗ Error running traceroute to {target}: {e}")
            return []
    
    def parse_traceroute(self, output: str, target: str) -> List[Dict]:
        """Parse traceroute output to extract hops, including before timeout"""
        hops = []
        ...
        for line in lines:
            # Skip empty lines and timeout markers
            ...
            rest_of_line = hop_match.group(2)
            
            # Parse multiple responses in same hop (load balancing)
            # Format: hostname (ip) time ms [hostname2 (ip2) time ms]
            hop_entries = []
            
            # Find all hostname (ip) patterns in the line
            pattern = r'([^\s]+)\s+\(([0-9.]+)\)'
            matches = re.findall(pattern, rest_of_line)
            
            if matches:
                for hostname, ip in matches:
                    # Classify router type
                    router_type = self.classify_router(hostname, ip)
                    ...
                    hop_entry = {
                        ...
                        'is_potential_gateway': router_type != 'unknown'
                    }
                    hop_entries.append(hop_entry)
                    
                    if router_type != 'unknown':
                        print(f"    🎯 [{router_type.upper()}] {hostname} ({ip})")
            
            # If we found any valid entries for this hop, add them
            hops.extend(hop_entries)
        
        if hops:
            print(f"    ✓ Captured {len(hops)} hops (including partial trace)")
        
        return hops
    
    def classify_router(self, hostname: str, ip: str) -> str:
        """Classify router type based on hostname patterns"""
        hostname_lower = hostname.lower()
        
        # TWAREN core routers
        if 'twaren' in hostname_lower:
            return 'twaren_core'
        
        # University border/gateway routers
        gateway_patterns = ['gw', 'gateway', 'border', 'br', 'wan', 'edge']
        if any(pattern in hostname_lower for pattern in gateway_patterns):
            return 'university_gateway'
        
        # University core routers
        core_patterns = ['core', 'backbone', 'dist']
        if any(pattern in hostname_lower for pattern in core_patterns):
            return 'university_core'
        
        # Check if it's within university range (heuristic: 140.x or 192.168.x often universities)
        if ip.startswith('140.') or ip.startswith('192.192.'):
            # If it has a .edu.tw domain
            if '.edu.tw' in hostname_lower:
                return 'university_internal'
    ...
    def analyze_results(self):
        ...
        for node_data in self.results['dns_lookups'].values():
            for key, value in node_data.items():
                if not key.endswith('_reverse') and '.' in str(value):
                    all_ips.add(value)
        
        for node_traces in self.results['traceroutes'].values():
            # Handle new structure (list of trace attempts)
            if isinstance(node_traces, list):
                for trace_attempt in node_traces:
                    for hop in trace_attempt.get('hops', []):
                        all_ips.add(hop['ip'])
            # Handle old structure (backward compatibility)
            else:
                for hop in node_traces.get('hops', []):
                    all_ips.add(hop['ip'])
        ...
        # Show success rate
        total_nodes = len(self.results['traceroutes'])
        successful_nodes = sum(1 for node_traces in self.results['traceroutes'].values() 
                              if isinstance(node_traces, list) and 
                              any(t['status'] == 'success' for t in node_traces))
        print(f"\nTraceroute Success Rate: {successful_nodes}/{total_nodes} nodes")
        
        # List nodes that failed all traceroutes
        failed_nodes = []
        for node_name, node_traces in self.results['traceroutes'].items():
            if isinstance(node_traces, list):
                if all(t['status'] == 'failed' for t in node_traces):
                    failed_nodes.append(node_name)
        
        if failed_nodes:
            print(f"\nNodes with no successful traces: {', '.join(failed_nodes)}")
            print("  → Consider manual investigation or checking firewall rules")
