...
class TWARENDiscovery:
    ...
    def discover_traceroutes(self, max_workers=5):
        """Phase 2: Traceroute discovery"""
        print("\n=== Phase 2: Traceroute Discovery ===\n")
        
        targets = []
        for node_name, domains in UNIVERSITIES.items():
            targets.append((node_name, domains[0]))  # Use primary domain
        
        for node_name, domains in CORE_NODES.items():
            targets.append((node_name, domains[0]))
        
        def trace_target(item):
            node_name, target = item
            print(f"Tracing route to {node_name} ({target})...")
            hops = self.run_traceroute(target)
            return node_name, target, hops
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            results = executor.map(trace_target, targets)
            
            for node_name, target, hops in results:
                self.results['traceroutes'][node_name] = {
                    'target': target,
                    'hops': hops
                }
